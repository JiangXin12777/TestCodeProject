...
public:
	...
	void OnPostEngineInit()
	{
		...
		if (GIsEditor)
		{
			...
			UPackage::PackageSavedWithContextEvent.AddRaw(this, &FEventsEditorModule::OnPackageSaved);
		}
	}
	...
	void OnPackageSaved(const FString& PackageFilename, UPackage* PackageObj, FObjectPostSaveContext ObjectSaveContext)
	{
	...
	virtual bool DeleteTagFromINI(TSharedPtr<FEventNode> TagNodeToDelete) override
	{
		...
		for (FName TagNameToDelete : TagsThatWillBeDeleted)
		{
			...
			// FIX blowpunch
			//AssetRegistryModule.Get().GetReferencers(TagId, Referencers, SearchableName); // UE 4.26
