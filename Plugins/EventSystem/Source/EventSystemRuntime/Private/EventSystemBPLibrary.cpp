...
void UEventSystemBPLibrary::NotifyEventByKeyVariadic(const FString& MessageId, UObject* Sender, UObject* Listener)
{
...
void UEventSystemBPLibrary::NotifyEventByKey(const FString& EventId, UObject* Sender, UObject* Listener, const TArray<FOutputParam, TInlineAllocator<8>>& Outparames)
{
	UGIEventSubsystem* System = UGIEventSubsystem::Get(Sender);
	if (System)
		System->NotifyEventWithParams(EventId, Sender, Listener, Outparames);
}
...
DEFINE_FUNCTION(UEventSystemBPLibrary::execNotifyEventByKeyVariadic)
{
	...
	P_FINISH
		...
		P_NATIVE_BEGIN
		UEventSystemBPLibrary::NotifyEventByKey(MessageId, Sender, Listener, OutParms);
	P_NATIVE_END
}
